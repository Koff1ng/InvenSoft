...
export default function PhysicalCountPage() {
  ...
  const approveCount = async (count: PhysicalCount) => {
    ...
    const { data: { user } } = await supabase.auth.getUser();
    if (!user) {
      setReviewError('Sesión no válida');
    ...
    for (const item of count.items) {
      if (item.counted_qty !== item.system_qty) {
        ...
        if (updErr) {
          setReviewError(updErr.message || 'Error al actualizar inventario');
          setReviewLoading(false);
          return;
        }

        const { error: insErr } = await supabase.from('inventory_updates').insert({
        ...
        if (insErr) {
          setReviewError(insErr.message || 'Error al registrar el movimiento');
          setReviewLoading(false);
          return;
        }
      }
    ...
    if (statusErr) {
      setReviewError(statusErr.message || 'Error al marcar el conteo como aprobado');
      setReviewLoading(false);
      return;
    }
  ...
  const rejectCount = async (count: PhysicalCount) => {
    ...
    if (error) {
      setReviewError(error.message || 'Error al rechazar el conteo');
      setReviewLoading(false);
      return;
    }
  ...
  return (
    <>
      ...
      <div className="max-w-5xl mx-auto p-4 sm:p-6">
        ...
        {/* Search bar for counts list */}
        {counts.length > 0 && (
          <div className="relative mb-4">
            <svg className="absolute left-3 top-1/2 -translate-y-1/2 text-[var(--text-subtle)]" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
            <input
              type="text"
              value={searchList}
              onChange={(e) => setSearchList(e.target.value)}
              className="input-field pl-9 w-full text-sm"
              placeholder="Buscar por área, nombre o estado..."
            />
            {searchList && (
              <button onClick={() => setSearchList('')} className="absolute right-3 top-1/2 -translate-y-1/2 text-[var(--text-muted)] hover:text-[var(--text)]">
        ...
        {counts.length === 0 ? (
          ...
          const q = searchList.toLowerCase().trim();
          const filtered = q ? counts.filter(c =>
          ...
          return filtered.length === 0 ? (
            <div className="text-center py-12 text-[var(--text-muted)]">
              <p className="text-sm">No se encontraron conteos para &quot;{searchList}&quot;</p>
            </div>
          ) : (
            ...
            <div className="relative bg-[var(--bg-card)] border border-[var(--border)] rounded-xl shadow-2xl w-full max-w-2xl mx-4 mb-8 overflow-hidden">
              ...
              <div className="p-6 space-y-4">
                ...
                <div className="relative">
                  <svg className="absolute left-3 top-1/2 -translate-y-1/2 text-[var(--text-subtle)]" width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
                  <input
                    type="text"
                    value={searchForm}
                  ...
                  />
                  {searchForm && (
                    <button onClick={() => setSearchForm('')} className="absolute right-3 top-1/2 -translate-y-1/2 text-[var(--text-muted)] hover:text-[var(--text)]">
                      <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
                    </button>
                  )}
                </div>
                ...
                <div className="max-h-[50vh] overflow-y-auto space-y-2">
                  {countItems.filter(item => {
                    if (!searchForm.trim()) return true;
                    return item.product_name.toLowerCase().includes(searchForm.toLowerCase().trim());
                  }).map((item) => {
                    const idx = countItems.findIndex(ci => ci.inventory_item_id === item.inventory_item_id);
                    return (
          ...
          <div className="fixed inset-0 z-50 flex items-start justify-center pt-4 overflow-y-auto">
            ...
            <div className="relative bg-[var(--bg-card)] border border-[var(--border)] rounded-xl shadow-2xl w-full max-w-2xl mx-4 mb-8 overflow-hidden">
              ...
              <div className="p-6 space-y-4">
                ...
                {reviewError && (
                  <div className="text-sm text-red-400 bg-red-900/15 px-3 py-2 rounded-lg">{reviewError}</div>
                )}
